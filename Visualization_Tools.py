...
def calories_over_range(dates_calories_sqlite_row, bmr):
	...
	for day in dates_calories_sqlite_row:
		...
		#Break list into two separate lists		
...
def surplus_deficit_over_range(dates_calories_sqlite_row, bmr):
	...
	#Now use bmr value to compute variances in positive, negative, or zero direction
	for count in calories_list_ma:
		difference = 0

		if count[0] == 0: #
			continue
		else:
			difference = count[0]-bmr
